...
* ## 46 билет
	...
	ODR - _one definition rule_ - сколько угодно обьявлений, но не более одного определения.

	Однако шаблоны нарушают ODR, т.к. для каждого типа, с которым была вызвана функция, компилятор генерируется новое определение:
	``` c++
	//// foo.h
...
* ## 47 билет
	...
	Include guards - защита от повторного подключения файла при использовании ```#include```. Использование их гарантирует, что при повторных подключениях, файл не будет подключаться.
	``` c++
	//// foo.h
	...
	#endif
	```
	Аналог include guards - pragma once. В стандарте ее нету, но она поддерживается почти всеми компиляторами. Только стоит быть осторожной с ней, т.к. задача определения идентичности файла по указанному пути, в общем случае, довольно трудная, и нет гарантий, что в компиляторе она хорошо решена.
	``` c++
	//// foo.h
...
* ## 48 билет
    ...
    **CV-квалификаторы** (const and volatile qualifiers):
